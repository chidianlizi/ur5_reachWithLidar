...
class Env(gym.Env):
    ...
    # test group 1    
    def _test_1(self):
        init_home = [0.2,0.32,0.3]
        init_orn = [np.pi,0,np.pi]
        target_position = [-0.3,0.65,0.3]
        ...
        obst_1 = p.createMultiBody(
            baseMass=0,
            baseVisualShapeIndex=self._create_visual_box([0.05,0.05,0.05]),
            baseCollisionShapeIndex=self._create_collision_box([0.05,0.05,0.05]),
            basePosition=[0.0,0.55,0.26],
    ...
    def _test_2(self):
        init_home = [-0.2,0.4,0.33]
        init_orn = [np.pi,0,np.pi]
        target_position = [0.3,0.5,0.2]
        target = p.createMultiBody(
                    baseMass=0,
                    baseVisualShapeIndex=p.createVisualShape(shapeType=p.GEOM_SPHERE, radius=0.02, rgbaColor=[1,0,0,1]),
                    basePosition=target_position,
        ...
        obst_1 = p.createMultiBody(
            baseMass=0,
            baseVisualShapeIndex=self._create_visual_box([0.002,0.05,0.05]),
            baseCollisionShapeIndex=self._create_collision_box([0.002,0.05,0.05]),
            basePosition=[0.2,0.45,0.25],
            baseOrientation=choice([0.707, 0, 0, 0.707])
        )
        obsts.append(obst_1)
